...
# 소개
  ...
  이전 직장에서는 Full Stack 개발자로 성장하고 싶어 서버든 클라이언트이든 마구잡이식으로 공부해왔었지만  
  MAP 환경에서 작업하면서 아래의 클라이언트 관련 문제들에 대해 고민을 하며 시도하던 와중에  
  오프라인 공부에서 React를 처음 공부하면서 지금껏 고민해왔던 부분들을 이미 다른 개발자들도 고민을 해왔고  
  저와 같은 고민을 했던 사람들이 사용하는 SPA 구조를 활용한 개발자가 되고 싶다는 생각이 들었고  
